# Загружаете исходний json
import json
with open("/home/daniil/PycharmProjects/HW.json", "r") as file:
    data = json.load(file)
print(type(data))
print(data)

# В списке словарей по ключам собираете полное ими персоны.
for employees in data['employee']:
    print(employees['firstName']), print(employees['lastName'])

# При помощи метода items из ключей и значений генерируете список кортежей пари ключ-значение
...
for k, v in data.items():
    ...
    for i, n in first_employee.items():
        print(type(n))
...
for k, v in data.items():
    ...
    for i, n in second_employee.items():
        print(type(n))
...
for k, v in data.items():
    ...
    for i, n in third_employee.items():
        print(type(n))
